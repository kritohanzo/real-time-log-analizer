...
def email_notificate(anomalous_event, users):
    ...
    print(
        f'START EMAIL NOTIFICATION TO RECIPIENTS {", ".join(recipient_list)}:', end=" "
    )
    try:
    ...
    for user in users:
        ...
        try:
            response = requests.post(
                MTS_SMS_API_URL,
                headers={"Authorization": f"Bearer {MTS_API_KEY}"},
                json={"number": MTS_NUMBER, "destination": destination, "text": text},
            )
...
def notificate_selector(anomalous_event, notification_types):
    ...
    for notification_type in notification_types:
        if notification_type.method == "email":
            users = notification_type.users.filter(email__isnull=False).exclude(
                email__exact=""
            )
            email_notificate(anomalous_event, users)
        elif notification_type.method == "sms":
            users = notification_type.users.filter(phone_number__isnull=False).exclude(
                phone_number__exact=""
            )
...
def get_datetime_from_line(line):
    ...
    if date_of_line and time_of_line:
        ...
        try:
            datetime_of_line = datetime.datetime.strptime(
                datetime_of_line, datetime_format
            ).replace(tzinfo=ZoneInfo(key="Asia/Yekaterinburg"))
...
def analyze_counter(line, search_pattern, log_file, datetime_of_line, ip_of_line):
    ...
    if created:
        ...
        if (
            ...
            + datetime.timedelta(
                hours=search_pattern.period_of_events.hour,
                minutes=search_pattern.period_of_events.minute,
                seconds=search_pattern.period_of_events.second,
            )
            > datetime_of_line
        ):
            anomalous_event.count_of_events += 1
    ...
    if search_pattern.pattern in line:
        if search_pattern.counter:
            ...
            anomalous_event = AnomalousEvent.objects.create(
                text=line,
                log_file=log_file,
                fact_datetime=datetime_of_line,
